...
Looking at the figure below, we can see that as $N$ increases, the optimal bandwidth decreases. Intuitively, with very few blocks the quartic fits are forced to cover large ranges of $x$, which leads to an underestimation of $\hat \theta_{22}$, pushing $\hat h$ upwards. As $N$ increases, the fitted model tracks better the local curvature, leading to higher $\hat\theta_{22}$ and lower $\hat \sigma^2$, which translate to lower $\hat h$.

![XXX](optimal_bandwidth.png){width="400"}

The figure below shows optimal values for $N$, estimated using Mallow's $C_p$. The plot shows that, as $n$ grows, larger values of $N$ are optimal. This happens because having more data means we can choose narrower buckets that still have many points.

Additionally plot 1 shows that with higher $n$ smaller bandwidths are preferred.

![XXX](mallows_cp.png){width="400"}

Figure 3 shows the result explained above. When $n=10,000$, using $N=5$ produces a wiggly result due to the high variance caused by having few points in each bucket. But when $n=1,000,000$, this effect is mitigated by having many points, and using $N=10$ produces much better results.

![XXX](density.png){width="600"}
